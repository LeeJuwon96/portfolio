...
MinIO 자체 장애에 대비해 주요 백업 bucket을 AWS S3로 복제하는 offsite 백업을 구현했다. AWS S3에는 Lifecycle 정책을 적용해 오래된 객체를 장기 보관 계층으로 전환하도록 구성했다.
...
- MinIO 장애 시 restic repository도 영향을 받을 수 있다는 한계가 있다.
...
| 백업 대상 | 사용 방식 | 확인 내용 |
|---|---|---|
| mgmt DB/Storage/Config | PostgreSQL dump와 archive | dump 및 압축 파일 생성 확인 |
| AIO etcd | etcd snapshot | snapshot 상태와 checksum 확인 |
| Kubernetes 리소스 | Velero | 백업 리소스와 복구 결과 확인 |
| mgmt host volume | restic | snapshot 생성과 파일 복구 확인 |

백업 작업별 실행 시간을 분리하는 구성은 이번 프로젝트에서 수행하지 않았다.
...
## 17. AWS S3 offsite 복제와 생명주기 정책

mgmt MinIO의 주요 백업 bucket을 AWS S3로 복제하고, S3 버킷에 생명주기 정책을 적용해 오래된 백업 객체의 장기 보관 비용을 줄이도록 구성했다.

- 백업 데이터의 보관 기간과 비용을 함께 고려
- 오래된 객체를 생명주기 정책으로 관리
- MinIO 장애에 대비해 주요 백업 bucket을 AWS S3로 외부 복제
...
6. MinIO 백업본은 AWS S3로 외부 복제하고, 생명주기 정책으로 장기 보관 비용을 관리한다.
...
- host volume은 restic
- 백업 저장소는 MinIO
- MinIO 백업본은 AWS S3로 외부 복제하고, 생명주기 정책을 적용해 장기 보관 비용 관리
