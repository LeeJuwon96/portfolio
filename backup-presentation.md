...
MinIO 자체 장애에 대비한 외부 저장소 구성은 이번 프로젝트의 구현 범위에 포함하지 않았다.
...
AWS S3 버킷에 생명주기 정책을 적용해 오래된 백업 객체의 장기 보관 비용을 줄이도록 구성했다.

- 백업 데이터의 보관 기간과 비용을 함께 고려
- 오래된 객체를 생명주기 정책으로 관리
- MinIO에서 AWS S3로 데이터를 전송하는 외부 복제는 구현하지 않음
...
6. AWS S3 생명주기 정책으로 장기 보관 비용을 관리한다.
...
- host volume은 restic
- 백업 저장소는 MinIO
- AWS S3에는 생명주기 정책을 적용해 장기 보관 비용 관리
