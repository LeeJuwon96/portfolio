...
## 6. AWS S3 offsite mirror

MinIO만 있으면 mgmt 서버 장애 시 백업 저장소까지 함께 영향을 받을 수 있습니다. 그래서 MinIO bucket을 AWS S3로 mirror했습니다.
...
| 항목 | 검증 방식 |
| --- | --- |
| Velero | namespace restore 절차 확인 |
| PostgreSQL | dump 기반 복구 확인 |
| restic | stage directory restore 확인 |
| etcd | checksum, snapshot status, restore rehearsal |

면접 답변:

> 운영 중인 공유 환경에서 전체 restore는 위험하기 때문에 발표장에서 직접 수행하지 않았습니다. 대신 안전한 범위의 복구 테스트와 etcd snapshot 검증을 진행했고, 실제 장애 시 따라 할 수 있는 Runbook으로 정리했습니다.
...
- 정기 복구 리허설 자동화
- RPO/RTO 수치화
- AWS S3 Lifecycle 기반 장기 보관 비용 최적화
- 백업 성공/실패 대시보드 구성
